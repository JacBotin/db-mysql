SELECT @@global.version as version_database,
       ...
       case @@global.log_warnings 
            ...
            when 0 then 'OK - log_warnings 0 enables logging of error messages.Equivalent log_error_verbosity = 1'
       ...
       case when @@global.log_warnings is null 
            then 'SET GLOBAL log_warnings=0;'
...
SELECT @@global.version as version_database,
       ...
       case when (select count(1) FROM INFORMATION_SCHEMA.PLUGINS WHERE PLUGIN_NAME = 'validate_password' and PLUGIN_STATUS = 'ACTIVE') > 0
            then 'no action needed'
            else concat("/* Run script mysqldb_cis_install_validate_password.sql*/")
       end as action_cmd  
union             
SELECT @@global.version as version_database,
       ...
       case when (select count(1) FROM INFORMATION_SCHEMA.PLUGINS WHERE PLUGIN_NAME = 'audit_log' and PLUGIN_STATUS = 'ACTIVE') > 0
            then 'no action needed'
            else concat("/* Run script mysqldb_cis_install_audit_log.sql */")
       end as action_cmd;	   