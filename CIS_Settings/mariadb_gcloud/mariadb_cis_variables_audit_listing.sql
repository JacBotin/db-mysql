...
select @@global.version as version_database,
       ...
       'set global datadir = ''/$path/datafiles/data''' as action_cmd
union
select @@global.version as version_database,
	   'File System Permissions' as cis_category, 
       ...
       'OK_NOK - Check location and access control' as diagnostic,
       'set global log_bin_basename = ''/$path/binlogs/mariadb-bin''' as action_cmd      
union       
select @@global.version as version_database,
	   'File System Permissions' as cis_category, 
...
select @@global.version as version_database,
	   ...
	   'OK_NOK - If necessary, check directory and access control configured in the variable relay_log_basename' as diagnostic,
	   'SET GLOBAL relay_log_basename = ''/$path/binlogs/relay-bin''' as action_cmd
union 	     
select @@global.version as version_database,
	   'File System Permissions' as cis_category, 
...
select @@global.version as version_database,
	   ...
	   'SET GLOBAL general_log_file = ''/$path/logs/mariadb-general.log'';' as action_cmd
union 	   
/* General */
select @@global.version as version_database,
...
SELECT @@global.version as version_database,
       ...
       case when (select count(1) FROM INFORMATION_SCHEMA.PLUGINS WHERE PLUGIN_NAME = 'validate_password' and PLUGIN_STATUS = 'ACTIVE') > 0
            then 'no action needed'
            else '/* Run script mysqldb_cis_install_validate_password.sql*/'
       end as action_cmd  
union             
/* Network */
select @@global.version as version_database,
