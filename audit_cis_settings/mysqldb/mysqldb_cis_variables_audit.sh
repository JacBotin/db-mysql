...
mysql -h $hostname -u root < mysqldb_cis_variables_audit_listing.sql | sed  's/\t/;/g' >> results_audit_$hostname/result_mysqldb_cis_variables_audit_listing.csv
#
# Lists action on variables that do not comply with the CIS recommendation. Must be added to the my.cnf file.
mysql -h $hostname -u root < mysqldb_cis_variables_audit_listing_nok_general.sql | sed  's/\t/;/g' >> results_audit_$hostname/result_mysqldb_cis_variables_audit_change_general.csv
#
...
mysql -h $hostname -u root < mysqldb_cis_variables_audit_listing_nok_path.sql | sed  's/\t/;/g' >> results_audit_$hostname/result_mysqldb_cis_variables_audit_change_path.csv
#
# Lists action on database users (other than root) who have super privileges. Only root user should have super privileges.
mysql -h $hostname -u root < mysqldb_cis_variables_audit_listing_nok_userprivilegies.sql | sed  's/\t/;/g' >> results_audit_$hostname/result_mysqldb_cis_variables_audit_change_userprivilegies.csv